// SPARKD MEME OF THE WEEK — CONTEST RULES
// contest-rules.js v1.2
...
(() => {
  ...
  const RULE_SECTIONS = [
    {
      title: "1. Contest Overview",
      items: [
        "SPARKD Meme of the Week is a weekly community meme contest.",
        "Each weekly contest moves automatically through a submission phase, a voting phase, and winner finalization.",
        "The live contest dates, phase, and countdown shown on the contest page are the public schedule. Server and database time control whether an action is actually accepted."
      ]
    },
    {
      title: "2. Meme Entry Requirements",
      items: [
        ...
        "A meme title is required and may contain up to 100 characters.",
        "The submission must be completed during the active submission window. Late new submissions are rejected by the server/database even if a browser page is still open."
      ]
    },
    {
      title: "3. Wallet & Burn Verification",
      items: [
        "The entrant uses a compatible Solana wallet such as Phantom to authorize the contest transaction.",
        "The contest verifies the SPARKD burn on-chain before the entry is finalized.",
        "A valid entry must have both DNA verification and burn verification.",
        "Canceling or rejecting the wallet transaction before it is broadcast does not create a completed entry or verified burn.",
        "Only burns associated with the correct active contest and submission window are accepted."
      ]
    },
    {
      title: "4. Submission Status & Moderation",
      items: [
        "New valid entries may initially appear with a pending status.",
        "Contest administrators may approve or reject a pending submission.",
        "A rejected meme is not eligible to receive contest votes.",
        "A meme does not currently need an approved status to appear as voting-eligible; it must have verified Forge DNA, a verified burn, and must not be rejected.",
        "Winner submissions are protected from ordinary moderation changes after winner finalization."
      ]
    },
    {
      title: "5. Voting Rules",
      items: [
    ...
    {
      title: "8. How the Winner Is Chosen",
      items: [
        "When voting closes, the eligible meme with the highest verified vote total wins.",
        "If two or more eligible memes are tied, the meme submitted earlier wins the tie.",
        "A contest must have at least one valid vote to produce a champion.",
        "If a completed contest receives zero valid votes, no artificial winner is created.",
        "Winner selection and contest rollover are performed automatically by the server-side contest lifecycle."
      ]
    },
    {
      title: "9. Champion & Hall of Fame",
      items: [
        "The first-place weekly winner becomes the Meme of the Week champion.",
        "The champion may be displayed in the large winner area on the contest page and in SPARKD's champion displays.",
        "Completed champions are preserved in the Meme Hall of Fame / Previous Champions area.",
        "The next weekly contest is created automatically after the previous contest is finalized."
      ]
    },
    {
  ...
  function addStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${ROOT_ID} { display:inline-flex; align-items:center; gap:12px; flex-wrap:wrap; }
      #${ROOT_ID} .sparkd-rules-week { margin:0; }
      #${ROOT_ID} .sparkd-rules-button {
        appearance:none; border:1px solid rgba(255,196,0,.55);
        background:rgba(24,24,28,.96); color:#fff; border-radius:999px;
        padding:9px 14px; font:inherit; font-size:12px; font-weight:900;
        letter-spacing:.06em; cursor:pointer; box-shadow:0 8px 24px rgba(0,0,0,.24);
        transition:transform .14s ease,border-color .14s ease,background .14s ease;
      }
      #${ROOT_ID} .sparkd-rules-button:hover,
      #${ROOT_ID} .sparkd-rules-button:focus-visible {
        transform:translateY(-1px); border-color:rgba(255,215,74,.95);
        background:#24242a; outline:none;
      }
      #${MODAL_ID} {
      ...
      #${MODAL_ID} .sparkd-rules-panel {
        width:min(920px,100%); max-height:min(88vh,960px); overflow:auto;
        background:#111116; color:#f5f5f7; border:1px solid rgba(255,255,255,.15);
      ...
      #${MODAL_ID} .sparkd-rules-header {
        ...
        background:rgba(17,17,22,.97); border-bottom:1px solid rgba(255,255,255,.12);
      }
      #${MODAL_ID} .sparkd-rules-title { margin:0; font-size:clamp(20px,3vw,28px); line-height:1.1; }
      #${MODAL_ID} .sparkd-rules-close {
        flex:0 0 auto; width:42px; height:42px; border-radius:50%;
        border:1px solid rgba(255,255,255,.18); background:#202027;
        color:#fff; font-size:22px; cursor:pointer;
      }
      #${MODAL_ID} .sparkd-rules-body { padding:20px; }
      #${MODAL_ID} .sparkd-rules-intro {
        margin:0 0 18px; padding:14px 16px; border-radius:14px;
        background:rgba(255,196,0,.08); border:1px solid rgba(255,196,0,.24);
        line-height:1.55;
      }
      #${MODAL_ID} .sparkd-rule-section { padding:18px 0; border-bottom:1px solid rgba(255,255,255,.09); }
      #${MODAL_ID} .sparkd-rule-section:last-child { border-bottom:0; }
      #${MODAL_ID} .sparkd-rule-section h3 { margin:0 0 12px; font-size:17px; }
      #${MODAL_ID} .sparkd-rule-section ul { margin:0; padding-left:21px; }
      #${MODAL_ID} .sparkd-rule-section li { margin:8px 0; line-height:1.55; color:#dedee5; }
      #${MODAL_ID} .sparkd-rules-footer {
        margin-top:18px; padding:14px 16px; border-radius:14px;
        background:rgba(255,255,255,.045); color:#c8c8d0; font-size:13px; line-height:1.5;
      }
      @media (max-width:640px) {
        #${MODAL_ID} { padding:10px; }
        #${MODAL_ID} .sparkd-rules-panel { max-height:94vh; border-radius:16px; }
        #${MODAL_ID} .sparkd-rules-header, #${MODAL_ID} .sparkd-rules-body { padding:15px; }
      }
      @media (prefers-reduced-motion:reduce) {
        #${ROOT_ID} .sparkd-rules-button { transition:none; }
      }
    `;
  ...
  function createModal() {
    ...
    intro.textContent = "These rules summarize how SPARKD Meme of the Week entries, voting, verification, and winner selection currently operate.";
    ...
    footer.textContent = "SPARKD may update contest rules as the contest system evolves. The rules displayed on the live contest page should be treated as the current public version.";
