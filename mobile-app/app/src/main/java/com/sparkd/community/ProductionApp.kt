...
@Composable private fun ContestEntry(wallet: WalletSession, repo: LiveRepository) {
    ...
    LazyColumn(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        item { Text("Entry requires one 2,000 SPARKD burn per wallet and contest. If that burn already exists, you may replace the Forge image and finalize the new verified PNG without burning again.", color = androidx.compose.ui.graphics.Color.LightGray) }
            ...
            Button({
                scope.launch {
                    ...
                    runCatching {
                        ...
                        prepared = burn
                        status = if (existingBurnSignature != null) "Existing verified contest burn found. Your selected Forge PNG can replace the previous image without another burn." else "Entry checks passed. Review the exact burn details below."
        ...
        prepared?.let { burn ->
            if (existingBurnSignature != null) {
                item { Card { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) { Text("Existing burn verified", fontSize = 20.sp, fontWeight = FontWeight.Black); Text("No additional SPARKD burn is required.", fontWeight = FontWeight.Bold, color = Green); Text("The currently selected verified Forge PNG will be used for final submission.") } } }
                item { Button(onClick = {
                    submitting = true
                    scope.launch {
                ...
                item { Card { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) { Text("Transaction review", fontSize = 20.sp, fontWeight = FontWeight.Black); Text("Burn amount: 2,000 SPARKD", fontWeight = FontWeight.Bold, color = Gold); Text("Token account: " + burn.tokenAccount.take(6) + "…" + burn.tokenAccount.takeLast(4)); Text("One signer • one burn instruction • Token-2022 verified"); Text("Your wallet will show the final transaction before signing. The burn is irreversible once confirmed on-chain.", color = androidx.compose.ui.graphics.Color.LightGray) } } }
    ...
    if (confirmBurn) {
        AlertDialog(
            ...
            text = { Text("This will ask your wallet to sign a transaction that permanently burns exactly 2,000 SPARKD. Do not approve unless you want to burn 2,000 SPARKD.") },
