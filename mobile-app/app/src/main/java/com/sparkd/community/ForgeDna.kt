...
object ForgeDna {
    ...
    fun extractAndVerify(png: ByteArray): ForgeDnaRecord {
        ...
        var forgeChunkStart = -1; var forgeChunkEnd = -1; var offset = 8
        while (offset + 12 <= png.size) {
            ...
            if (type == "tEXt") {
                val data = png.copyOfRange(offset + 8, offset + 8 + length); val zero = data.indexOf(0.toByte())
                if (zero > 0 && String(data, 0, zero, Charsets.UTF_8) == "SPARKD-FORGE") {
                    payloads += String(data, zero + 1, data.size - zero - 1, Charsets.UTF_8); forgeChunkStart = offset; forgeChunkEnd = offset + 12 + length
                }
            }
        ...
        val record = ForgeDnaRecord(
            forge=json.getString("forge"), version=json.getString("version"), memeID=json.getString("memeID"), DNA=json.getString("DNA"),
            imageFingerprint=json.getString("imageFingerprint"), imageLock=json.getString("imageLock"), created=json.getString("created"), contract=json.getString("contract"),
            creatorID=json.getString("creatorID"), wallet=json.getString("wallet"), reputation=json.getInt("reputation"), signature=json.getString("signature"),
        ...
        if (record.pngFingerprint != null) {
            ...
            check(pngFingerprint(originalPng) == record.pngFingerprint) { "The meme PNG changed after leaving SPARKD Meme Forge." }
        } else {
