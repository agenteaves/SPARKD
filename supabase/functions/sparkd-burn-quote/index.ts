...
async function getSupply(rpcUrl: string) {
  ...
  if (!Number.isFinite(supply) || supply <= 0 || !Number.isInteger(decimals) || decimals < 0) throw new Error("Invalid SPARKD supply response.");
  return { supply, rawSupply: String(value?.amount ?? ""), decimals };
}
async function getJupiterPrice() {
  ...
  const row = (await response.json())?.[SPARKD_MINT];
  ...
  if (!Number.isFinite(liquidity) || liquidity < MIN_LIQUIDITY_USD) throw new Error("SPARKD liquidity is unavailable or below the adaptive-price safety floor.");
  return { usdPrice, liquidity, blockId: Number.isFinite(blockId) ? blockId : null, source: apiKey ? "jupiter-price-v3" : "jupiter-price-v3-lite" };
}
...
serve(async (req) => {
  ...
  try {
    ...
    try { price = await getJupiterPrice(); } catch (error) { fallbackReason = error instanceof Error ? error.message : String(error); }
    const burnSparkd = price ? Math.round(clamp(TARGET_USD / price.usdPrice, MIN_BURN, MAX_BURN)) : LEGACY_BURN;
    const mode = price ? "adaptive" : "legacy-safe";
    const multiplier = 10n ** BigInt(supply.decimals);
    return json({
      success: true, quoteVersion: "1.1.0", mint: SPARKD_MINT, mode, burnSparkd,
      rawBurnAmount: String(BigInt(burnSparkd) * multiplier), targetEntryUsd: TARGET_USD,
      estimatedEntryUsd: price ? burnSparkd * price.usdPrice : null, priceUsd: price?.usdPrice ?? null,
      priceSource: price?.source ?? null, priceLiquidityUsd: price?.liquidity ?? null, priceBlockId: price?.blockId ?? null,
      remainingSupplySparkd: supply.supply, remainingSupplyRaw: supply.rawSupply, decimals: supply.decimals,
      minBurnSparkd: MIN_BURN, maxBurnSparkd: MAX_BURN, fallbackReason,
      observedAt: new Date(observedAtMs).toISOString(), quoteExpiresAt: new Date(observedAtMs + (price ? QUOTE_TTL_MS : 30000)).toISOString(),
      serverAuthoritative: true, clientRateLimitInputAccepted: false, readOnly: true, burnsPerformed: false, transfersPerformed: false
    });
